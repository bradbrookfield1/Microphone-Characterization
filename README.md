...
### Choosing My Frequency Range
In order to get the best results out of this program, rated frequency ranges of all devices is absolutely crucial. In my case, there are three main devices that need to be considered, which are the audio analyzer, the level meter, and the speakers. The listed audio analyzer has a range of 20 Hz - 20 kHz, so unless one has top notch quality studio speakers or an extremely accurate level meter, this range can basically be ignored. The level meter has a range of 31.5 Hz - 8 kHz though, which if your speakers involve a subwoofer, this is likely to be the biggest frequency bottleneck. The speakers mentioned above reach a range of 30 Hz - 20 kHz with the subwoofer. Without it, the range is 90 Hz - 20 kHz. In my case, since the subwoofer reaches lower than the level meter, the level meter's range of 31.5 Hz - 8 kHz is the largest range that can be fully trusted. However, note that if I did not have the subwoofer, the trusted range would be 90 Hz - 8 kHz. In short, one is limited by their most constricting range.
